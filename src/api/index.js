...
export const getLatestLogin = async(user_id) => {
  try {
    const response = await axiosInstance.get(`${process.env.REACT_APP_API_BASE_URL}/get_lookup/latest_employee_clock_in?user_id=${user_id}`)
    return response.data
  } catch (err) {
    // handle errors here
...
export const handlePostTimeLog = async (project_id, user_id, isMidnight=false, isResumeLog=false, resumeLogId=null) => {
  const obj = {
    time_in: moment().utc(),
    application_type : 'desktop',
